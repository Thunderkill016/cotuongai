...
function humanoid(color: THREE.Color, elite = false) {
  ...
  add(
    g,
    mesh(new THREE.SphereGeometry(0.15, 12, 8), skin, [
      0,
      elite ? 0.98 : 0.91,
      0,
    ]),
  ...
  add(
    g,
    mesh(new THREE.CylinderGeometry(0.17, 0.19, 0.11, 8), metal, [
      0,
      elite ? 1.11 : 1.04,
      0,
    ]),
  ...
  if (elite) {
    ...
    add(
      g,
      mesh(
        new THREE.CylinderGeometry(0.026, 0.026, 0.92, 6),
        dark,
        [0.25, 0.66, 0],
        [0, 0, -0.08],
      ),
...
function horse(color: THREE.Color) {
  ...
  add(
    g,
    mesh(
      new THREE.CapsuleGeometry(0.23, 0.54, 5, 10),
      hide,
      [0, 0.55, 0],
      [Math.PI / 2, 0, 0],
    ),
  ...
  add(
    g,
    mesh(
      new THREE.CylinderGeometry(0.12, 0.16, 0.43, 8),
      hide,
      [0, 0.78, -0.27],
      [-0.48, 0, 0],
    ),
  ...
  for (const x of [-0.16, 0.16]) {
    add(
      g,
      mesh(new THREE.CylinderGeometry(0.045, 0.05, 0.48, 6), dark, [
        x,
        0.23,
        -0.18,
      ]),
  ...
  add(
    g,
    mesh(
      new THREE.CylinderGeometry(0.14, 0.18, 0.46, 8),
      cloth,
      [0, 0.97, 0.1],
    ),
  );
...
function elephant(color: THREE.Color) {
  ...
  for (const x of [-0.22, 0.22]) {
    add(
      g,
      mesh(new THREE.CylinderGeometry(0.07, 0.08, 0.46, 7), hide, [
        x,
        0.25,
        -0.14,
      ]),
...
function chariot(color: THREE.Color) {
  ...
  for (const x of [-0.39, 0.39])
    add(
      g,
      mesh(
        new THREE.CylinderGeometry(0.2, 0.2, 0.09, 12),
        metal,
        [x, 0.27, 0],
        [0, 0, Math.PI / 2],
      ),
...
function cannon(color: THREE.Color) {
  ...
  add(
    g,
    mesh(
      new THREE.CylinderGeometry(0.11, 0.17, 0.88, 12),
      metal,
      [0, 0.59, -0.06],
      [Math.PI / 2, 0, 0],
    ),
...
function commander(color: THREE.Color, guard = false) {
  ...
  add(
    g,
    mesh(new THREE.ConeGeometry(0.07, guard ? 0.31 : 0.47, 8), plume, [
      0,
      guard ? 1.32 : 1.42,
      0,
    ]),
...
function addBanner(root: THREE.Group, x: number, z: number, red: boolean) {
  ...
  const flag = mesh(
    ...
    material(red ? COLORS.red : COLORS.black),
    [x + 0.24, 1.02, z],
  );
...
export function Battlefield3DEnhanced({
  ...
  useEffect(() => {
    ...
    const rim = mesh(
      new THREE.BoxGeometry(10.45, 0.2, 11.45),
      material("#62442b", 0.86),
      [0, -0.21, 0],
    );
    root.add(rim);
    const base = mesh(
      new THREE.BoxGeometry(9.9, 0.25, 10.9),
      material(COLORS.wood, 0.9),
      [0, -0.08, 0],
    );
    root.add(base);
    const river = mesh(
    ...
    for (const baseRank of [0, 7]) {
      line(
        root,
        squareToWorld(`d${baseRank}`, flipped),
        squareToWorld(`f${baseRank + 2}`, flipped),
        0.03,
      );
      line(
        root,
        squareToWorld(`f${baseRank}`, flipped),
        squareToWorld(`d${baseRank + 2}`, flipped),
        0.03,
      );
    }
